  ...
  return (
    <div className="border-b border-border last:border-0">
      <button
        ...
        className="flex w-full items-center gap-2 px-3 py-2 text-xs font-medium text-muted transition-colors hover:text-text"
      >
...
export function CampoRecord({
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1">
      <span className="flex w-[118px] flex-none items-center gap-1.5 text-xs text-muted">
        <span className="text-faint">{icona}</span>
        <span className="truncate">{etichetta}</span>
      </span>
      <span className={cn("min-w-0 flex-1 truncate text-xs", !haValore && "text-faint")}>
      ...
      <div className="flex flex-none items-center gap-1 overflow-x-auto border-b border-border px-2">
        {schede.map((s) => (
          <button
            ...
            className={cn(
              "flex flex-none items-center gap-1.5 border-b-2 px-2 py-2 text-xs transition-colors",
              attiva === s.chiave
          ...
          >
            ...
            {s.conteggio !== undefined && s.conteggio > 0 && (
              <span className="rounded bg-surface2 px-1 text-xxs text-faint">
                {s.conteggio}
              </span>
            )}
          </button>
