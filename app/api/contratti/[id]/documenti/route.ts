...
export async function POST(
  ...
  const esiste = await prisma.contratto.findUnique({ where: { id }, select: { id: true } });
  if (!esiste) {
  ...
  // L'UUID nella chiave evita che due file con lo stesso nome si sovrascrivano.
  const chiave = `contratti/${id}/${randomUUID()}-${file.name}`;
